...
* @param {Number} [opts.r=1] - number of failures until the experiment is stopped
...
function mgf( t, options ) {
	...
	if ( isMatrixLike( t ) ) {
		...
		return mgf5( out, t, opts.r, opts.p );
	}
	if ( isTypedArrayLike( t ) ) {
		...
		return mgf6( out, t, opts.r, opts.p );
	}
	if ( isArrayLike( t ) ) {
		// Handle deepset first...
		if ( opts.path ) {
			opts.sep = opts.sep || '.';
			return mgf4( t, opts.r, opts.p, opts.path, opts.sep );
		}
		...
		if ( opts.accessor ) {
			return mgf3( out, t, opts.r, opts.p, opts.accessor );
		}
		return mgf2( out, t, opts.r, opts.p );
	}
	return NaN;
} // end FUNCTION mgf()
