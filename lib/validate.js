...
* @param {Number} [options.r=1] - number of failures until the experiment is stopped
...
function validate( opts, options ) {
	...
	if ( options.hasOwnProperty( 'r' ) ) {
		...
		if ( !isPositive( opts.r ) ) {
			return new TypeError( 'mgf()::invalid option. `r` parameter must be a positive number. Option: `' + opts.r + '`.' );
		}
	}
